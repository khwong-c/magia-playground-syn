...
function parse_source_positions(str: string): Digitaljs.SourcePosition[] {
    ...
    for (const entry of str.split('|')) {
        ...
        const [from, to] = pos.split('-').map(s => s.split('.').map(v => Number(v))).map(([line, column]) => ({
            line,
            column
        }));
...
function yosys_to_digitaljs(data: Yosys.Output, portmaps: Portmaps, options: ConvertOptions = {}): {
    [key: string]: Digitaljs.Module
} {
    const out = {};
    for (const [name, mod] of Object.entries(data.modules)) {
...
export async function process(obj: Yosys.Output, options: Options = {}): Promise<Output> {
    try {
...
export const handler = async (event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
    ...
    const obj: Yosys.Output = JSON.parse(input);
